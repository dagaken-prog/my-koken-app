...
# --- Supabase接続設定 ---
# .streamlit/secrets.toml または Streamlit Cloud の Secrets から読み込む
try:
...
def main():
    ...
    if menu == "利用者情報・活動記録":
        ...
        if not df_persons.empty and '現在の状態' in df_persons.columns:
            ...
            if df_active.empty: df_active = df_persons.copy() # データがなければ全員
        else:
    ...
    # --- 2. 関係者・連絡先 ---
        ...
        if target_name:
            ...
            if not df_rel.empty:
                my_rel = df_rel[df_rel['person_id'] == pid]
