...
def process_import(file_obj, table_name, mapping_dict, id_column=None):
    ...
    try:
        ...
        # 一括処理したいが、安全のため1件ずつUpsert（件数が少ない想定）
        # ※大量データなら bulk insert を検討すべき
        for rec in records:
...
def calculate_age(born):
    ...
    try:
        born_date = pd.to_datetime(born).date()
...
def main():
    ...
    # データ読み込み（必要な画面でのみ読み込む最適化も可能だが、簡便のため一括）
    ...
    if menu == "利用者情報・活動記録":
        ...
        if not df_persons.empty and '現在の状態' in df_persons.columns:
            # None混じりの比較エラー回避のためfillna
            df_active = df_persons[df_persons['現在の状態'].fillna('').astype(str) == '受任中'].copy()
        ...
        if selection.selection.rows:
            ...
            if not df_related.empty:
                kp_df = df_related[
                    (df_related['person_id'] == int(current_pid)) & 
            ...
            with st.expander("➕ 新しい活動記録を追加する", expanded=False):
                with st.form("new_act_form", clear_on_submit=True):
                    ...
                    in_type = col2.selectbox("活動", ["面会", "打ち合わせ", "電話", "メール", "行政手続き", "財産管理", "その他"])
            ...
            if not df_activities.empty:
                my_acts = df_activities[df_activities['person_id'] == current_pid].sort_values('記録日', ascending=False)
                ...
                if st.session_state.edit_activity_id:
                    ...
                    with st.container(border=True):
                        ...
                        with st.form("edit_act_form"):
                            ...
                            try:
                                idx = ["面会", "打ち合わせ", "電話", "メール", "行政手続き", "財産管理", "その他"].index(edit_row['活動'])
                            except:
                                idx = 6
                            ed_type = st.selectbox("活動", ["面会", "打ち合わせ", "電話", "メール", "行政手続き", "財産管理", "その他"], index=idx)
                            c3, c4, c5 = st.columns(3)
        ...
        if target_name:
            ...
            with st.expander("➕ 新しい関係者を追加", expanded=False):
                with st.form("new_rel"):
                    c1, c2 = st.columns(2)
                    r_type = c1.selectbox("種別", ["親族", "ケアマネ", "施設", "病院", "行政", "その他"])
            ...
            st.markdown("---")
            if not df_related.empty:
                my_rel = df_related[df_related['person_id'] == pid]
        ...
        if target_name:
            ...
            with st.expander("➕ 財産追加", expanded=False):
                with st.form("new_asset"):
                    c1, c2 = st.columns(2)
                    a_type = c1.selectbox("種別", ["預貯金", "現金", "不動産", "保険", "負債", "その他"])
            ...
            if not df_assets.empty:
                my_assets = df_assets[df_assets['person_id'] == pid]
    ...
    # --- 4. 利用者情報登録 ---
        ...
        with st.expander("➕ 新規登録", expanded=True):
            with st.form("new_person"):
                ...
                p_type = c2.selectbox("類型", ["後見", "保佐", "補助", "任意"])
            ...
            for _, row in df_persons.iterrows():
                with st.expander(f"{row['氏名']} ({row['類型']})"):
                    with st.form(f"edit_p_{row['person_id']}"):
                        e_stat = st.selectbox("状態", ["受任中", "終了"], index=["受任中", "終了"].index(row['現在の状態']) if row['現在の状態'] in ["受任中", "終了"] else 0)
    ...
    # --- 6. データ管理・移行 (CSVインポート) ---
        ...
        custom_header("データ管理")
        st.info("Supabaseへのデータ移行用です。既存のIDは維持されます。")
        
        # ★全テーブルのバックアップ/インポートに対応
        tabs = st.tabs(["利用者", "活動", "財産", "関係者", "システム"])
